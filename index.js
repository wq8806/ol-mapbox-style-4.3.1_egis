...
function getLayerBackground(layer) {
  ...
  background.id = 'olms-bg-' + paint['background-opacity'] + paint['background-color'];
  if (paint['background-color'] !== undefined) {
    const bg = getValue(background, 'paint', 'background-color', 10, emptyObj);
    // return Color.parse(bg).toArray();
    const colorArr = Color.parse(bg).toArray();
    colorObj.r = Math.round(colorArr[0]);
    colorObj.g = Math.round(colorArr[1]);
    colorObj.b = Math.round(colorArr[2]);
    colorObj.a = Math.round(colorArr[3] * 255 / colorArr[3]);
  }
  return colorObj;
}
/**
...
export function getBackground(glStyle) {
  for (let i = 0; i < glStyle.layers.length; i++) {
    const l = glStyle.layers[i];
    if (l.type == 'background') {
      return getLayerBackground(l);
    }
  }
  return colorObj;
}
