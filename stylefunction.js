 ...
 */
export function stylefunction3d(glStyle, sources, resolutions = defaultResolutions, spriteData, spriteImageUrl, getFonts) {
  ...
  const styleFunction = function (feature, level) {
            ...
            if (spriteImage && spriteData && spriteData[icon]) {
              ...
              if (type == 2) {
                ...
                if (geom.getFlatMidpoint) {
                  ...
                  //     Math.pow(((extent[2] - extent[0])/8) / resolution, 2),
                  //     Math.pow(((extent[3] - extent[1])/8) / resolution, 2))
                  // );

                  const size = Math.sqrt(Math.max(
                    Math.pow(((extent[2] - extent[0]) / 8), 2),
                    Math.pow(((extent[3] - extent[1]) / 8), 2))
                  );
                  if (size > (150 * 1)) {
    ...
    for (let k = 0; k < sources.length; k++) {
      ...
      if (typeof source == 'string' && layer.source == source ||
        ...
        if (!mapboxSource) {
          ...
          if (!source) {
            continue;
            // throw new Error(`Source "${mapboxSource}" is not defined`); //TS-GIS
          }
          const type = source.type;
          if (type !== 'vector' && type !== 'geojson') {
            throw new Error(`Source "${mapboxSource}" is not of type "vector" or "geojson", but "${type}"`);
